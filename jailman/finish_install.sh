...
then
	...
	cp "${includes_dir}"/dashboard.toml /mnt/"${global_dataset_config}"/"${1}"/temp/${1}.toml
else
	echo "wildcard set, using wildcard config..."
	cp "${includes_dir}"/dashboard_wildcard.toml /mnt/"${global_dataset_config}"/"${1}"/temp/${1}.toml
fi
...
iocage exec "${1}" sed -i '' "s|placeholderdashboardhost|${domain_name}|" /config/temp/${1}.toml
...
then
	...
	iocage exec "${1}" sed -i '' "s|placeholderwildcard|${cert_wildcard_domain}|" /config/temp/${1}.toml
fi
